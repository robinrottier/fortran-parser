...
program
   : commentStatement* (executableUnit commentStatement* EOL*)+ EOL*
   ;
...
executableStatement
   : (assignmentStatement | gotoStatement | ifStatement | doStatement | continueStatement | stopStatement | pauseStatement | readStatement | writeStatement | printStatement | rewindStatement | backspaceStatement | openStatement | closeStatement | endfileStatement | inquireStatement | callStatement | returnStatement | formatStatement)
   ;
...
gotoStatement
   : (GO | GOTO) (unconditionalGoto | computedGoto | assignedGoto)
   ;
...
readStatement
   : READ ((formatIdentifier ((COMMA ioList) +)? | LPAREN iexpr COMMA formatIdentifier (COMMA END ASSIGN iexpr)? (COMMA ERR ASSIGN iexpr)? RPAREN) (ioList (COMMA ioList) +)? )
   ;
